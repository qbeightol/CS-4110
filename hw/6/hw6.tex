...
\begin{enumerate}
	...
	\item 
		...
		Since $e_1: \tau \rightarrow \tau',$ by canonical forms, we know that $e_1 = \lambda x:\tau \text{ } e_2,$ or, that $e_1$ is a lambda abstraction.\\
		
		We know that $\textsf{fix } (\lambda x:\tau \text{ } e_2)$ steps, using the small-step semantics rule given to us in problem 3. Therefore, the progress lemma holds, concluding this sub-case and our proof.\\
		\end{itemize}
		...
		\begin{prooftree}
			...
			\UnaryInfC{$ \vdash (\lambda x: \tau_x.e_\text{body}): \tau \rightarrow \tau$}
			\UnaryInfC{$ \vdash \textsf{fix } (\lambda x: \tau_x.e_\text{body}): \tau$}
